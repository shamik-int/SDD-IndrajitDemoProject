...
**Remediation pass, 2026-09-21 (Author, Indrajit Bhandari):** an audit
cross-referenced all 30 items above against the actual spec/plan/ADR
artifacts. Roughly half (G1-02, 03, 05, 08, 09, 10, 13, 14, 15, 19, 20, 23,
25) already had a defensible, documented v1 answer — mostly via ADR-0004 (no
backend) or ADR-0005 (local-only auth) — that this register did not
distinguish from the genuinely-open items. The remaining items were
partially or fully open; **G1-07, G1-12, G1-17, G1-22, G1-24, and G1-28 have
now been resolved at spec/plan level** (`employee-internal-transfer.spec.md`
v1.5, `employee-registration-login.spec.md` v1.2,
`plans/employee-internal-transfer.plan.md` v3) — see those files' v1.5/v1.2
changelog notes for what changed and why. **This remediation has not been
reviewed or approved by Shamik Bhattacharya.** The Gate decision above
("Not approved... development remains blocked") stands until he reviews
these changes; this note records what changed, not a self-approval.
